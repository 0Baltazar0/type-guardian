...
import { zodStringDefiner } from "./zod-natives/zodString";
import { zodNumberDefiner } from "./zod-natives/zodNumber";
import { zodIntegerDefiner } from "./zod-natives/zodInteger";
import { zodBooleanDefiner } from "./zod-natives/zodBoolean";
type YamlType =
  | number
  | string
  | boolean
...
export class ZodBundler {
  ...
  unWindTypeDeclaration(root: TypeDeclaration): string {
    if ("type" in root == false) {
      if ("oneOf" in root) {
        return `z.any().refine(zodXorRefine(${root.oneOf
  ...
  unWindObject(root: ObjectType) {
    ...
    if (props) {
      ...
      return `z.object({ ${values
        .map((v) => {
          return `${v[0]}:${v[1]}${
            required?.includes(v[0]) ? "" : ".optional()"
          }`;
        })
        .join(",")}})`;
    }
  ...
  unWindArray(root: ArrayType) {
    if (root.items) {
      return `z.array(${this.unWindTypeDeclaration(root.items)})`;
    }
    return "z.array()";
  }
  ...
  unWindSchemaType(root: ExplicitTypeDeclaration) {
    switch (root.type) {
      ...
      case "string":
        return zodStringDefiner(root);
      case "number":
        return zodNumberDefiner(root);
      case "integer":
        return zodIntegerDefiner(root);
      case "boolean":
        return zodBooleanDefiner(root);
    }
  }
    ...
    if (mergedParameters.length > 0) {
      ...
      const queryString = mergedParameters
        ...
        .map(
          (par) =>
            `${par[0].name}:${par[1]}${
              (par[0] as QueryParameter).allowEmptyValue == true
                ? ".nullable()"
                : ""
            }`
        )
  ...
  unWindRequestBody(root: RequestBodies | RefType): string {
    ...
    if ("$ref" in root) return `requestBody:${this.refToComponent(root.$ref)}`;

    return `requestBody: ${this.unWindContentEntry(root.content)}`;
  }
  ...
  translatePaths(): string | undefined {
    ...
    if (paths) {
      return `export const paths = {${Object.keys(paths)
        .map((key) => {
  ...
  unWindResponseEntry(root: ResponsesType): string {
    if ("$ref" in root) {
      ...
      return root.content ? this.unWindContentEntry(root.content) : "z.any()";
    }
  }
